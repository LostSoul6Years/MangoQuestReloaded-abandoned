...
import me.Cutiemango.MangoQuest.manager.config.QuestConfigSaver;
...
public class QuestCommand
{
		...
		if (args.length == 1) {
			...
			switch (args[1]) {
				...
				case "option":
					...
					if (QuestNPCManager.getOption(args[3]) == null || !QuestValidater.validateNPC(args[2]) || !QuestUtil.getData(target)
							.isNearNPC(Main.getHooker().getNPC(args[2])))
						return;
				...
				case "trade":
					target = sender;
					if (args.length == 4) {
						target = Optional.ofNullable(Bukkit.getPlayer(args[3])).orElse(sender);
					}
			...
			switch (args[1]) {
				...
				case "cquit":
					if (!quest.isQuitable()) {
						QuestChatManager.error(sender, I18n.locMsg(sender,"QuestQuitMsg.Denied"));
						return;
					}
					qd.quitQuest(quest);
					QuestBookGUIManager.openJourneyMenu(sender);
	...
	private static void sendHelp(Player p) {
		...
		QuestChatManager.info(p, I18n.locMsg(p,"CommandHelp.List"));
		QuestChatManager.info(p, I18n.locMsg(p,"CommandHelp.View"));
		QuestChatManager.info(p, I18n.locMsg(p,"CommandHelp.Take"));
		QuestChatManager.info(p, I18n.locMsg(p,"CommandHelp.Quit"));
	}
