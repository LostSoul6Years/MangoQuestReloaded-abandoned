...
public class ConversationEditorCommand
{
	...
	public static void execute(Player sender, String[] args) {
		if (!sender.hasPermission("MangoQuest.ConversationEditor")) {
			QuestChatManager.error(sender, I18n.locMsg(sender,"CommandInfo.NoPermission"));
			return;
		}
			...
			switch (args[1]) {
				...
				case "saveall":
					...
					if (conv.getActions().isEmpty()) {
						QuestChatManager.error(sender, I18n.locMsg(sender,"EditorMessage.ConversationEmpty"));
						return;
					} else if (conv instanceof StartTriggerConversation && ((StartTriggerConversation) conv).getQuest() == null) {
						QuestChatManager.error(sender, I18n.locMsg(sender,"EditorMessage.TriggerQuestEmpty"));
						return;
					}
					QuestConfigManager.getSaver().saveConversation(conv);
					QuestConversation.synchronizeLocal(conv);
					QuestChatManager.info(sender, I18n.locMsg(sender,"EditorMessage.ConvSaveCfgSuccess", conv.getName()));
					QuestChatManager.info(sender, I18n.locMsg(sender,"EditorMessage.ConvSaveSevSuccess", conv.getName()));
					break;
				case "sc":
				case "savecfg":
					if (!ConversationEditorManager.checkEditorMode(sender, true))
			...
			switch (args[1]) {
				case "select":
					if (ConversationManager.getConversation(args[2]) == null) {
						QuestChatManager.error(sender, I18n.locMsg(sender,"CommandInfo.ConversationNotFound"));
						return;
					}
					if (ConversationEditorManager.checkEditorMode(sender, false)) {
						if (!confirm.contains(sender.getName())) {
							QuestChatManager.error(sender, I18n.locMsg(sender,"EditorMessage.QuitEditing"));
					...
					QuestChatManager.info(sender, "&c" + I18n.locMsg(sender,"EditorMessage.Entered"));
					return;
				case "new":
					CommandNewAction.execute(conv, sender, args);
					return;
