		...
		if (args.length == 1) {
			...
			switch (args[1]) {
				...
				case "remove":
					...
					return;
				case "help":
					QuestChatManager.info(sender, I18n.locMsg(sender,"EditorMessage.UseCommand"));
					return;
				case "exit":
				...
				case "saveall":
					...
					if (q.getStages().isEmpty() || q.getStage(0).getObjects().isEmpty()) {
						QuestChatManager.error(sender, I18n.locMsg(sender,"EditorMessage.QuestEmpty"));
						return;
					}
					QuestConfigManager.getSaver().saveQuest(q);
					Quest.synchronizeLocal(q);
					QuestChatManager.info(sender, I18n.locMsg(sender,"EditorMessage.QuestSaveCfgSuccess", q.getQuestName()));
					QuestChatManager.info(sender, I18n.locMsg(sender,"EditorMessage.QuestSaveSevSuccess", q.getQuestName()));
					break;
				case "sc":
				case "savecfg":
					if (!QuestEditorManager.checkEditorMode(sender, true))
