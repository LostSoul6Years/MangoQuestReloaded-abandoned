			...
			switch (args[1]) {
				...
				case "takequest":
					if (cp.getConversation() instanceof StartTriggerConversation) {
						...
						cp.retrieve();
						cp.getCurrentPage().add(trigger.getAcceptMessage(sender)).changeLine();
				...
				case "denyquest":
					if (cp.getConversation() instanceof StartTriggerConversation) {
						StartTriggerConversation trigger = (StartTriggerConversation) cp.getConversation();
						cp.retrieve();
						cp.getCurrentPage().add(trigger.getDenyMessage(sender)).changeLine();
