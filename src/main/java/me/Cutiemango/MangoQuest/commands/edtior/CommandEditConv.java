...
public class CommandEditConv
{
	...
	private static void editNPC(QuestConversation conv, Player sender, String[] args) {
		if (args.length == 3) {
			EditorListenerHandler
					.register(sender, new EditorListenerObject(ListeningType.NPC_LEFT_CLICK, "mq ce edit npc", Syntax.of("I", "[NPCID]", "")));
	...
	private static void editFriendConvPoint(QuestConversation conv, Player sender, String[] args) {
		if (args.length == 3) {
			EditorListenerHandler.register(sender,
					new EditorListenerObject(ListeningType.STRING, "mq ce edit fconvp", Syntax.of("D", I18n.locMsg(sender,"Syntax.Number"), "")));
			QuestBookGUIManager.openInfo(sender, I18n.locMsg(sender,"EditorMessage.EnterValue"));
		} else if (args.length == 4) {
			if (conv instanceof FriendConversation)
	...
	private static void editQuest(StartTriggerConversation conv, Player sender, String[] args) {
		if (args.length == 3)
			...
			conv.setQuest(q);
			QuestChatManager.info(sender, I18n.locMsg(sender,"EditorMessage.ObjectRegistered", q.getQuestName()));
			ConversationEditorManager.editConversation(sender);
		}
	}
	...
	private static void editAction(QuestConversation conv, Player sender, String[] args, String type) {
		if (args.length == 4) {
				...
				switch (type) {
					case "act":
						...
						if (list.size() - 1 >= index) {
							list.set(index, new QuestBaseAction(sender,act, null));
			...
			if (act == EnumAction.NPC_TALK) {
				EditorListenerHandler.register(sender,
						new EditorListenerObject(ListeningType.STRING, "mq ce edit " + type + " " + index + " " + act.name(),
								Syntax.of("S@D", I18n.locMsg(sender,"Syntax.NPCTalk"), "@")));
				QuestBookGUIManager.openInfo(sender, I18n.locMsg(sender,"EditorMessage.NPCTalk"));
				return;
			}
			EditorListenerHandler.register(sender,
					new EditorListenerObject(ListeningType.STRING, "mq ce edit " + type + " " + index + " " + act.name(), null));
			QuestBookGUIManager.openInfo(sender, I18n.locMsg(sender,"EditorMessage.EnterValue"));
			...
			switch (type) {
				case "act":
					List<QuestBaseAction> list = conv.getActions();
					if (list.size() - 1 >= index) {
						list.set(index, new QuestBaseAction(sender,act, s));
						conv.setActions(list);
					}
					break;
				case "acceptact":
