...
public class CommandRemoveQuest
{
	...
	private static void removeRequirements(Quest q, Player sender, String[] args) {
		...
		if(args.length == 4) {
			switch(t) {
			case LEVEL:{
				q.getRequirements().put(RequirementType.LEVEL, null);
				QuestEditorManager.editQuestRequirement(sender);
				return;
			}
			case MONEY:{
				q.getRequirements().put(RequirementType.MONEY, null);
				QuestEditorManager.editQuestRequirement(sender);
				return;
			}
			}
		...
		if (args.length == 5) {
			switch (t) {
				...
				case WORLD_TIME:
				case SERVER_TIME:
					q.getRequirements().put(t, new ArrayList<Long>());
					break;
				case PLACEHOLDER_API:
				   ((HashMap<String,String>)q.getRequirements().get(t)).remove(args[4]);
				   break;
				case MCMMO_LEVEL:
					((Map<PrimarySkillType,Integer>)q.getRequirements().get(t)).remove(mcMMO.p.getSkillTools().matchSkill(args[4]));
					break;
			}
			QuestEditorManager.editQuestRequirement(sender);
			QuestChatManager.info(sender, I18n.locMsg(sender,"EditorMessage.ObjectRemoved"));
		}
	}
	...
	private static void removeQuest(Player sender, String[] args) {
		if (args.length == 4) {
			if (QuestUtil.getQuest(args[3]) != null) {
				...
				QuestChatManager.info(sender, I18n.locMsg(sender,"EditorMessage.QuestRemoved", target.getQuestName()));
