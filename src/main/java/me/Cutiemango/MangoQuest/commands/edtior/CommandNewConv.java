...
public class CommandNewConv
{
	...
	private static void setInternalID(Player p, String[] args) {
		if (args.length == 3) {
			QuestBookGUIManager.openInfo(p, I18n.locMsg(p,"EditorMessage.NewConv.EnterID"));
		...
		QuestChatManager.info(p, I18n.locMsg(p,"EditorMessage.NameRegistered", args[3]));
		ConversationEditorManager.createConversation(p);
	}

	private static void setConvName(Player p, String[] args) {
		if (args.length == 3) {
			QuestBookGUIManager.openInfo(p, I18n.locMsg(p,"EditorMessage.NewConv.EnterName"));
	...
	private static void setConvNPC(Player p, String[] args) {
		if (args.length == 3) {
			QuestBookGUIManager.openInfo(p, I18n.locMsg(p,"EditorMessage.NewConv.EnterNPC"));
			EditorListenerHandler.register(p,
					new EditorListenerObject(ListeningType.NPC_LEFT_CLICK, "mq ce newconv npc", Syntax.of("N", I18n.locMsg(p,"Syntax.NPCID"), "")));
			return;
		}
		if (args[3].equalsIgnoreCase("cancel")) {
			ConversationEditorManager.createConversation(p);
		...
		QuestChatManager.info(p, I18n.locMsg(p,"EditorMessage.NPCRegistered", me.Cutiemango.MangoQuest.compatutils.Minecraft.MinecraftCompatability.getName(npc)));
		ConversationEditorManager.createConversation(p);
	}
	...
	private static void create(Player p) {
		...
		if (conv.getInternalID() != null && conv.getName() != null && conv.getNPC() != null) {
			QuestChatManager.info(p, I18n.locMsg(p,"EditorMessage.NewConv.Successful", conv.getName()));
			ConversationEditorManager.editConversation(p);
		} else {
			QuestChatManager.info(p, I18n.locMsg(p,"EditorMessage.NewConv.Failed"));
			ConversationEditorManager.createConversation(p);
		}
	}
}
