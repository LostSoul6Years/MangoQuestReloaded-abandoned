		...
		if (args.length == 4)
			...
			if (EnumAction.NO_OBJ_ACTIONS.contains(act)) {
				switch (acttype) {
					case "act":
						...
						list.add(index, new QuestBaseAction(sender,act, null));
			...
			if (act == EnumAction.NPC_TALK) {
				EditorListenerHandler.register(sender,
						new EditorListenerObject(ListeningType.STRING, "mq ce new " + acttype + " " + index + " " +act.name(),
								Syntax.of("S@I", I18n.locMsg(sender,"Syntax.NPCTalk"), "@")));
				QuestBookGUIManager.openInfo(sender, I18n.locMsg(sender,"EditorMessage.NPCTalk"));
				return;
			}
			EditorListenerHandler.register(sender,
					new EditorListenerObject(ListeningType.STRING, "mq ce new " + acttype + " " + index + " " +act.name(), null));
			...
			switch (acttype) {
				case "act":
					List<QuestBaseAction> list = conv.getActions();
					list.add(index, new QuestBaseAction(sender,act, s));
					conv.setActions(list);
					break;
				case "acceptact":
					list = ((StartTriggerConversation) conv).getAcceptActions();
