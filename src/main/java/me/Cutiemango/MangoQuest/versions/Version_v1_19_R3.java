...
public class Version_v1_19_R3 implements VersionHandler{
	...
	public TextComponent textFactoryConvertItem(final ItemStack item, boolean finished) {
		...
		if (item != null) {
			...
			if (StupidReflection.callMethodWithObjectArgs(CraftItemStack.asNMSCopy(item),"u").isEmpty() && false) {
	...
	public ItemStack addGUITag(ItemStack item) {
		...
		NBTTagCompound stag = StupidReflection.callMethodWithObjectArgs(nmscopy,"u").isPresent() ? ((NBTTagCompound)StupidReflection.callMethodWithObjectArgs(nmscopy,"u").get()): new NBTTagCompound();
	...
	public boolean hasGUITag(ItemStack item) {
		net.minecraft.world.item.ItemStack  nmscopy = CraftItemStack.asNMSCopy(item);
		NBTTagCompound tag = StupidReflection.callMethodWithObjectArgs(nmscopy,"u").isPresent()? ((NBTTagCompound)StupidReflection.callMethodWithObjectArgs(nmscopy,"u").get()): new NBTTagCompound();
		//same
	...
	public void playNPCEffect(Player p, Location location) {
		...
		PacketPlayOutWorldParticles packet = new PacketPlayOutWorldParticles(Particles.X, false,
