...
public class StupidReflection {
   ...
   //TODO:call method with only access level, and class of args
   
   public static Optional<Object> callMethodWithMethodDeclaration(Object caller,AccessLevel level,Class<?> returnedObj, Object... callee){
   try {
		   ...
		   if(callee.length != 0 && callee != null) {
			   ...
			   for(int i = 0; i < callee.length;i++) {
				   if(callee[i] != null) {
					   ...
					   calleetypes[i] = getSafeTypeObject(callee[i]);
			   ...
			   for(Method m:callerClass.getDeclaredMethods()) {
				  // if(m.getName().equals(methodName)) {
				   ...
				   default:{
				   	...
				   	if( m.getReturnType() != returnedObj) {
				   		continue;
				   	}
		  ...
		  if(toCall != null) {
			 ...
			 // DebugHandler.log(2, "called method: "+toCall.toGenericString());
			  ...
			  //if(caller instanceof NBTTagCompound) {
				//  DebugHandler.log(3, "called method: "+toCall.toGenericString());
			 // }
...
//     boolean isGeneric = hasTypeParameters || hasGenericSuperclass || hasGenericSuperinterface;
   ...
   public static Class<?> getSafeTypeObject(Object obj){
   ...
   public static Class<?> getSafeType(Class<?> type){
   ...
   public static Object getFieldWithTypeAndAccess(Object obj, Class<?> clazz, AccessLevel level){
