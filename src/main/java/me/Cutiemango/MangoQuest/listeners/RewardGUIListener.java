	...
	public static void onInventoryClick(InventoryClickEvent e) {
		...
		if (e.getCurrentItem() != null) {
			...
			if (inv.getTitle().contains(QuestChatManager.translateColor(I18n.locMsg(p,"QuestReward.RewardTitle")))) {
				...
				p.closeInventory();
			} else if (QuestEditorManager.checkEditorMode(p, false) && inv.getTitle().equals(QuestChatManager.translateColor(I18n.locMsg(p,"QuestReward.RewardEditTitle") + ChatColor.stripColor(QuestEditorManager.getCurrentEditingQuest((Player)e.getWhoClicked()).getQuestName())))) {
			...
			} else if (QuestEditorManager.checkEditorMode(p, false) && inv.getTitle().contains(QuestChatManager.translateColor(I18n.locMsg(p,"QuestReward.ChoiceEditTitle")))) {
