...
public class Syntax
{
	...
	// F - Positive and negative decimal value (including 0)
	// B - Boolean
	...
	public static Syntax regexCreate(String customRegex,String rejectionMsg) {
		Syntax syn = new Syntax();
		syn.order = customRegex;
		syn.regex = customRegex;
		syn.desc=rejectionMsg;
		return syn;
	}
		...
		for (int i = 0; i < order.toCharArray().length; i++) {
			if (order.toCharArray()[i] == 'N') {
				...
				if (Main.getHooker().getNPC(inputArray.get(i)) == null) {
					QuestChatManager.error(p, I18n.locMsg(p,"SyntaxError.NPCNotValid"));
					return false;
				}
			} else if (order.toCharArray()[i] == 'W') {
				if (!QuestValidater.isWorld(inputArray.get(i))) {
					QuestChatManager.error(p, I18n.locMsg(p,"SyntaxError.WorldNotValid"));
					return false;
				}
			}
		}
