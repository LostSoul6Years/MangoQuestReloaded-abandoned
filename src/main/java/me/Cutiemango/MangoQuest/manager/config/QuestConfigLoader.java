...
public class QuestConfigLoader {
	...
	public void loadConfig() {
		...
		if(!config.contains("enableautotranslation")) {
			config.set("enableautotranslation", true);
			config.save();
		}
		
		ConfigSettings.ENABLE_AUTO_TRANSLATION = config.getBoolean("enableautotranslation");
		if(ConfigSettings.ENABLE_AUTO_TRANSLATION) {
			DebugHandler.log(1, "fetching "+SupportedLanguage.valueOf(ConfigSettings.LOCALE_USING.toLanguageTag().replace("-", "_")).name());
			TranslatableService.fetch(SupportedLanguage.valueOf(ConfigSettings.LOCALE_USING.toLanguageTag().replace("-", "_")),true);
			for(SupportedLanguage lang:ConfigSettings.PLAYER_LOCALE_CHOICES.keySet()) {
				DebugHandler.log(1,"fetching "+lang.name());
				TranslatableService.fetch(lang);
			}
		}
		
		if(!config.contains("fullautotranslation")) {
			config.set("fullautotranslation", false);
			config.save();
		}
		
		ConfigSettings.FULL_AUTO_TRANSLATION = config.getBoolean("fullautotranslation");
