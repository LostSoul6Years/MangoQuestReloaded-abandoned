...
public class ConversationProgress
{
	public ConversationProgress(Player p, QuestConversation qc) {
		...
		// Init the first page
		currentBook.addPage(0, ConversationManager.generateNewPage(p,qc));
		currentBook.removePage(1);
	}

	// actions that needs player interaction or wait
	public static final List<EnumAction> STOP_ACTIONS = Arrays
			.asList(EnumAction.BUTTON, EnumAction.WAIT, EnumAction.CHOICE, EnumAction.FINISH, EnumAction.TAKE_QUEST, EnumAction.EXIT);
	...
	public static final List<EnumAction> DELAYED_ACTIONS = Arrays
			.asList(EnumAction.BUTTON, EnumAction.CHANGE_PAGE, EnumAction.NPC_TALK, EnumAction.SENTENCE, EnumAction.TAKE_QUEST, EnumAction.CHOICE);
	...
	public void nextAction() {
		...
		if (rushed) {
			...
			if (actQueue.isEmpty()) {
				...
				// still more actions
				// if the action type is not displaying text, execute immediately
	...
	public void rush() {
		if (rushDisabled) {
			QuestChatManager.error(owner, I18n.locMsg(owner,"Conversation.SkipDisabled"));
			ConversationManager.openConversation(owner, this);
			return;
		}
		rushed = true;
	...
	public void finish(boolean questFinish) {
		...
		ConversationManager.finishConversation(owner);
		QuestUtil.getData(this.owner).finishTalkToNPCObjective(conv.getNPC(),conv.getInternalID());
		QuestUtil.getData(this.owner).talkToNPC(conv.getNPC());
	...
	public QuestBookPage getCurrentPage() {
		if (currentBook.getFirstPage().isOutOfBounds()) {
			currentBook.pushNewPage(owner,conv);
