...
public class QuestChoice
{
	...
	public void apply(ConversationProgress cp) {
		...
		cp.getCurrentPage()
				.add(new InteractiveText(cp.getOwner(),   question.getText()).showText(I18n.locMsg(cp.getOwner(),"Conversation.ClickToAnswer")).clickCommand("/mq conv openchoice"));
	}

	public void choose(Player p, int i) {
		if (i > choices.size()) {
			QuestChatManager.error(p, I18n.locMsg(p,"Conversation.AnswerDenied"));
			return;
		}
		int count = 0;
