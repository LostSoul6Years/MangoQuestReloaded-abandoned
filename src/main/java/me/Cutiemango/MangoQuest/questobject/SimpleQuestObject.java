...
public abstract class SimpleQuestObject
{
	...
	protected TextComponent toTextComponent(Player p,String s, boolean isFinished, Object... args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i] instanceof ItemStack)
				text.addExtra(TextComponentFactory.convertItemHoverEvent(p,(ItemStack) args[i], isFinished));
				...
				// QuestObjectKillMob
					...
					String translation = QuestUtil.translate(p,( EntityType)args[i]);
					if(translation.contains("transentity")) {
						...
						text.addExtra(QuestUtil.translate(p,( EntityType) args[i]));
					}
					
				}
			}
		if (block != null) {
			String translation = QuestUtil.translate(p, block);
	...
	protected TextComponent toTextComponent(boolean autoSpacing, Player p,String s, boolean isFinished, Object... args) {
		...
		if (block != null)
			text.addExtra(color + QuestUtil.translate(p, block));
		text.addExtra(color + left);
