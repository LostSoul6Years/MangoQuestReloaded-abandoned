...
public class QuestObjectEnterCommand extends NumerableObject implements EditorObject{

	private String filter="/say hello (.+)";
	//the displayname used in locMsg
	private String displayName="你好指令:/say hello";
    private boolean useRegex = false;
	@Override
public String toDisplayText(Player p) {
		if(useRegex) {
			return I18n.locMsg(p,"QuestObject.EnterCommand",displayName,amount+"");
		}else {
			return I18n.locMsg(p,"QuestObject.EnterCommand",filter,amount+"");
		}
	}
	...
	public String getObjectName() {		
		return I18n.locMsg(null,"QuestObjectName.EnterCommand");
	}

	@Override
	public TextComponent toTextComponent(Player p, boolean isFinished) {
		if(useRegex) {
			return super.toTextComponent(p,ChatColor.stripColor(I18n.locMsg(null,"QuestObject.EnterCommand")), isFinished, displayName, amount);
		}else {
			return super.toTextComponent(p,ChatColor.stripColor(I18n.locMsg(null,"QuestObject.EnterCommand")), isFinished, filter, amount);
		}
	}
