...
public class QuestObjectTameMob extends NumerableObject implements EditorObject
{
	public QuestObjectTameMob() {
	}

	public QuestObjectTameMob(EntityType t, int i, String customname) {
		type = t;
		amount = i;
		customName = customname;
	}

	public QuestObjectTameMob(String mmMob, int i) {
		mtmMob = mmMob;
		amount = i;
	...
	public String getObjectName() {
		return I18n.locMsg(null,"QuestObjectName.TameMob");
	}
	...
	public TextComponent toTextComponent(Player p,boolean isFinished) {
		if (hasCustomName())
			return super.toTextComponent(p,ChatColor.stripColor(I18n.locMsg(null,"QuestObject.TameMob")), isFinished, amount, customName);
		else
...
public String toDisplayText(Player p) {
		if (hasCustomName())
			return I18n.locMsg(p,"QuestObject.TameMob", Integer.toString(amount), QuestChatManager.translateColor(customName));
		else
	...
	public boolean load(QuestIO config, String path) {
		if (config.getString(path + "MythicMob") != null) {
			...
			mtmMob = id;
			customName = config.getString(path+"MobName"); //+ "MobType"
