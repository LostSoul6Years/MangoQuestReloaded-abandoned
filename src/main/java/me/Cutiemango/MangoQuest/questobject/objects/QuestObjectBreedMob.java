...
public class QuestObjectBreedMob extends NumerableObject implements EditorObject
{
	public QuestObjectBreedMob() {
	}

	public QuestObjectBreedMob(EntityType t, int i, String customname) {
		type = t;
		amount = i;
	...
	//error-prone
    public QuestObjectBreedMob(String mmMob, int i) {
		mtmMob = mmMob;
		amount = i;
	...
	public String getObjectName() {
		return I18n.locMsg(null,"QuestObjectName.BreedMob");
	}
	...
	public TextComponent toTextComponent(Player p,boolean isFinished) {
		if (hasCustomName())
			return super.toTextComponent(p,ChatColor.stripColor(I18n.locMsg(null,"QuestObject.BreedMob")), isFinished, customName, amount);
		else
...
public String toDisplayText(Player p) {
		if (hasCustomName())
			return I18n.locMsg(p,"QuestObject.BreedMob", QuestChatManager.translateColor(customName), Integer.toString(amount));
		else
			return I18n.locMsg(p,"QuestObject.KillMob", Integer.toString(amount), mtmMob != null && hasCustomName() ? getCustomName() : type != null ? type.name() : rawType);
	}
	...
	public void formatEditorPage(Player p,FlexibleBook page, int stage, int obj) {
		...
		page.add(new InteractiveText(p,I18n.locMsg(null,"QuestEditor.Edit")).clickCommand("/mq e edit object " + stage + " " + obj + " mobname"))
				.changeLine();
		page.add(I18n.locMsg(null,"QuestEditor.MobType") + (type == null ? "N/A" : QuestUtil.translate(type)));
	...
	public boolean load(QuestIO config, String path) {
		if (config.getString(path + "MythicMob") != null) {
			...
			if(typeName != null){type = EntityType.valueOf(typeName);}else{type=null;}
	...
	public boolean receiveCommandInput(Player sender, String type, String obj) {
		switch (type) {
			...
			case "mtmmob":
				...
				setMythicMob(obj);
				setCustomName(Main.getHooker().getMythicMobsAPI().getMythicPlaceholderedName(obj));
