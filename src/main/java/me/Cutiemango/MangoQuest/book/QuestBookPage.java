...
public class QuestBookPage {
	...
	private static final double MAXIMUM_CHAR_PER_LINE = 18.4D;//19 original
	...
	public QuestBookPage add(TextComponent t) {
		...
		//DebugHandler.log(5,"very original");
		...
		boolean colourSymbol = false;
		int index = 0;
		
		//should not be a problem for now since magic can happen
		//List<Integer> colorPoll = new ArrayList<>();
		
		for (char c : sanitized.getText().toCharArray()) {
			Pair<Double, Double> pair = CHARACTER_SIZEMAP.getOrDefault(Character.UnicodeBlock.of(c),
					new Pair<>(1d, 1d));
			...
			if (colourSymbol) {
				colourSymbol = false;
				charSize = 0;
			} else if (c == '§' && (Character.isDigit(sanitized.getText().toCharArray()[index + 1])||Character.isAlphabetic(sanitized.getText().toCharArray()[index+1]))) {
				colourSymbol = true;
				charSize = 0;
			}
	...
	public void changeLine() {
		if (saved.toPlainText().length() != 0) {
			page.addExtra(saved);
			//DebugHandler.log(5, "size used for "+saved.toLegacyText()+": "+size);
			saved = new TextComponent();
