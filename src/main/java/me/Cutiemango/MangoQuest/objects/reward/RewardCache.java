...
public class RewardCache
{
	...
	public void finish() {
		...
		QuestChatManager.info(owner, I18n.locMsg(owner,"QuestReward.RewardReceived"));
	...
	public void openGUI() {
		Inventory inv = Bukkit.createInventory(null, 27, I18n.locMsg(owner,"QuestReward.RewardTitle"));

		for (int i = 0; i < reward.getChoiceAmount(); i++) {
			ItemStack item = itemButton(owner,reward.getChoice(i), i);
	...
	private ItemStack glassPane(int amount) {
		...
		glassPane.setName(I18n.locMsg(owner,"QuestReward.RewardGlassPane", Integer.toString(amount)));
		return glassPane.get();
	}

	private ItemStack itemButton(Player p,RewardChoice rc, int index) {
		...
		for (ItemStack item : rc.getItems()) {
			lore.add(QuestChatManager.translateColor("&f- " + QuestRewardManager.getItemName(owner,item)));
		}
		lore.add(ChatColor.BLACK + "" + index);
		button.setName("&f" + QuestRewardManager.getItemName(owner,firstItem));
		button.setLore(lore);
		return button.get();
	}
