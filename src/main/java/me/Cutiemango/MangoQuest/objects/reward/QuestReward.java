	...
	public void executeReward(Player p) {
		if (hasMoney()) {
			if (Main.getHooker().hasEconomyEnabled()) {
				...
				QuestChatManager.info(p, I18n.locMsg(p,"QuestReward.GiveMoneyReward", Double.toString(money)));
			}
		}

		if (hasExp()) {
			p.giveExp(experience);
			QuestChatManager.info(p, I18n.locMsg(p,"QuestReward.GiveExpReward", Double.toString(experience)));
		}
		...
		if (hasSkillAPIExp()) {
			if (Main.getHooker().hasSkillAPIEnabled()) {
				...
				QuestChatManager.info(p, I18n.locMsg(p,"QuestReward.GiveRPGExpReward", Integer.toString(skillapiExp)));
			}
		}
		...
		if (hasQRPGExp()) {
			if (Main.getHooker().hasQuantumRPGEnabled()) {
				...
				QuestChatManager.info(p, I18n.locMsg(p,"QuestReward.GiveRPGExpReward", Integer.toString(qrpgExp)));
			}
		}
	}
}
